...
impl AppContainer {
    ...
    pub async fn wire(config: &Config) -> Result<Self, DIError> {
        ...
        let master_key_vec = hex::decode(&config.master_key)
            .map_err(|e| DIError::MissingConfig(format!("invalid master key: {}", e)))?;
        let master_key: [u8; 32] = master_key_vec
            .try_into()
            .map_err(|_| DIError::MissingConfig("master key must be 32 bytes".to_string()))?;
